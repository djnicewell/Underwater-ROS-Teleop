...
void kbDataCallback(const std_msgs::Int32MultiArray& command){
  ...
  //If rolling left
  if(command.data[8] == 1 || command.data[0] == 1 || command.data[1] == 1 || command.data[2] == 1 || command.data[3] == 1){
    cmd_pololu(0,0);
    return;
  }
...
void loop(){
  ...
  /* code reads command from central computer */
  ...
  switch(state){ 
    case 0:
      Bangbang(target_angle,motor_speed);
      break;
    default:
      // stop all motors
      cmd_pololu(1,30);
